  ...
  return (
    <Styled.Content isFirstItem={isFirstItem}>
      <Styled.WrapperIconAndUserName>
        <Styled.TransactionIcon icon={icon} isMobile={isMobile()} />
        <Styled.Text fontColor={colors.PRIMARY_GRAY}>{username}</Styled.Text>
      </Styled.WrapperIconAndUserName>

      <Styled.WrappperTransactionLabel>
        <Styled.TransactionLabel fontColor={colors.PRIMARY_GRAY_DARKEN}>
          {label}
      ...
      <Styled.WrapperTotal>
        <Styled.Text space fontColor={total?.color}>
          {total?.prefix}
        </Styled.Text>
        <Styled.Text
          bold={!isRefunded}
          fontColor={total?.color}
